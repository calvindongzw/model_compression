...
def finetune_after_prune(net, epochs, batch_size, lr, reg, log_every_n, model_dir):
    print('==> Preparing data..')
    transform_train = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(),
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for name, module in net.named_modules():
                if isinstance(module, PrunedConv):
                    grad = module.conv.weight.grad.data.cpu().numpy()
                    grad = np.where((1 - module.mask.cpu().numpy()) == 0, 0, grad)
                    module.conv.weight.grad.data = torch.from_numpy(grad).to(device)
                if isinstance(module, PruneLinear): 
                    grad = module.linear.weight.grad.data.cpu().numpy()
                    grad = np.where((1 - module.mask.cpu().numpy()) == 0, 0, grad)
                    module.linear.weight.grad.data = torch.from_numpy(grad).to(device)
